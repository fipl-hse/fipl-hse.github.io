...
2. Click ``Code``, select ``HTTPS`` and click the copy button.
   ...
   .. figure:: ../_static_ctlr/fork_update/copy_original_repo_url.png
      :alt: copy repo url
...
4. Run ``git remote add upstream <link-to-main-repository>``.
   ...
   .. image:: ../_static_ctlr/fork_update/add_upstream.png

5. Run ``git fetch upstream``.

   .. image:: ../_static_ctlr/fork_update/fetch_upstream.png
...
6. Run ``git merge upstream/main --no-edit``.
   ...
   .. image:: ../_static_ctlr/fork_update/merge_upstream.png

...
.. note::
   ...
   расширение Python. Перейдите во вкладку Extensions (``Ctrl+Shift+X``).
   Введите название расширения в поисковую строку.
   Нажмите Install рядом с нужным расширением.

.. attention::

   В Visual Studio Code доступно автосохранение файлов.
   Чтобы его включить, выберите File -> Auto Save. Для
   выбора режима необходимо настраивать автосохранение.
   Для этого нажмите сочетание клавиш ``Ctrl + ,``, после
   чего в настройках выберите один из доступных режимов.
...
1. Откройте терминал в среде разработки Visual Studio Code с помощью ``Ctrl + ```:

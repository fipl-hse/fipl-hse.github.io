...
Реализуйте методы :py:meth:`lab_4_auto_completion.main.TrieNode.get_value`
и :py:meth:`lab_4_auto_completion.main.TrieNode.set_value`. Первый из них
должен возвращать относительную частоту соответствующего n-грамма в корпусе.
Второй метод кладёт новозаданную частоту в атрибут ``self._value``.
...
Переопределите метод :py:meth:`lab_4_auto_completion.main.TrieNode.add_child`. Это
нужно, чтобы создавались экземпляры класса
:py:class:`lab_4_auto_completion.main.TrieNode`,
а не :py:class:`lab_4_auto_completion.main.TrieNode`. Метод
:py:meth:`lab_4_auto_completion.main.TrieNode.add_child` должен добавлять
...
Переопределите метод :py:meth:`lab_4_auto_completion.main.TrieNode.__str__`.
Строка должна возвращаться в формате ``TrieNode(name, value)``.
...
Шаг 9.2.1. Соотнести узел копируемого ребёнка c узлом принимающего дерева
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Вам предлагается производить слияние постепенно, с наиболее простых единиц
и случаев на более комплексные. На данном шаге Вам потребуется произвести
соотнесение копируемого узла ребёнка с принимающим узлом дерева.

В общем случае это означает скопировать в узел родителя принимающего
дерева узел ребёнка копируемого дерева.

Реализуйте защищённый метод
:py:meth:`lab_4_auto_completion.main.DynamicNgramLMTrie._assign_child`,
который получает на вход узел родителя, идентификатор потенциального
узла ребёнка и частотное значение для заполнения информации о ребёнке.

Как Вы помните из структуры``TrieNode``, они
определяются через внутреннюю информацию и узлы его детей. Есть три
варианта, как можно найти нужного ребёнка:

1. Если в передаваемом на вход узле родителя уже есть информация о его
   узлах-детях и среди них нашёлся узел с идентификатором, совпадающим
   с переданным в метод, то возвращается именно этот узел ребёнка.

2. Если в метод был передан идентификатор потенциального узла ребёнка,
   который не существует среди детей узла родителя, то
   то добавьте узел ребёнка с данным идентификатором в детей родителя.
   Этот ребёнок и должен быть возвращён.

3. В любом ином случае, когда невозможно найти или создать узел ребёнка,
   верните последний добавленный в список узлов детей узел.


.. note:: Если аргумент с дополнительным значением для ребёнка не передан,
          то есть является `None`, то не нужно заполнять это поле.
          Если же он передан, то Вам необходимо использовать метод
          :py:meth:`lab_4_auto_completion.main.TrieNode.set_value`,
          чтобы добавить эту информацию в узел ребёнка.

.. important:: В методе также обязательно использовать методы
               :py:meth:`lab_4_auto_completion.main.TrieNode.get_name` и
               :py:meth:`lab_4_auto_completion.main.TrieNode.add_child`,
               наследуемые через ``TrieNode``.


Шаг 9.2.2. Создать копию дерева в корневом узле динамического дерева
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Теперь перейдём к обработке деревьев для последующего слияния.
Защищённый метод
:py:meth:`lab_4_auto_completion.main.DynamicNgramLMTrie._copy_tree`,
который Вам потребуется реализовать на данном шаге, отвечает за то,
чтобы целиком копировать одно дерево (от его корневого узла ``from_root``)
в корневой узел другого дерева (``to_root``).

Такое копирование понадобится нам для того, чтобы создать первичную
ветвь в динамическом дереве на основе одного из n-граммных деревьев.
От неё уже будут ответвляться фрагменты других деревьев.

Копирование должно происходить постепенно: от корневого узла, к
узлам его детей, затем к узлам их детей, и так далее.

Возьмём за текущие копируемый и принимающий узлы корневые узлы,
передаваемые в метод. Сохраним их пару в список пар, требующих
проверку на наличие узлов детей.

Алгоритм заполнения ``to_root`` следующий:

1. Для каждой последней пары текущих копируемого и принимающего узла
   Вам необходимо найти узлы детей копируемого узла.

2. Обработайте узлы детей.

   1. Если они существуют, для каждого узла копируемого ребёнка
      произведите соотнесение с принимающем узлом, как в предыдущем шаге.
      Полученный принимающий узел ребёнка сохраните в список пар,
      требующих проверку на наличие узлов детей, вместе с копируемым
      узлом ребёнка.

   2. Если они не существуют, переходите к последнему из добавленных
      в список узлов.

3. Выполняйте заполнение, пока у Вас есть узлы, сохранённые на
   проверку. То есть, те, чьи дети существуют, но Вы их ещё
   не копировали в принимающее дерево.

В итоге работы алгоритма аргумент ``to_root`` будет
полностью копировать содержание дерева ``from_root``, и мы сможем
использовать такое скопированное дерево для дальнейшего слияния.

.. note:: Почему это работает без возвращения переменных?
...
.. important:: В данном шаге необходимо использовать следующие методы:
   :py:meth:`lab_4_auto_completion.main.TrieNode.get_children`,
   :py:meth:`lab_4_auto_completion.main.TrieNode.get_name`,
   :py:meth:`lab_4_auto_completion.main.DynamicNgramLMTrie._assign_child`.


Шаг 9.2.3. Добавить поддерево как ветвь в динамическом дереве
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Реализуйте защищённый метод
:py:meth:`lab_4_auto_completion.main.DynamicNgramLMTrie._merge_tree_level`.


Шаг 9.2.4. Расширить ``NGramTrieLanguageModel`` новым функционалом
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Шаг 9.2.5. Выполнить слияние деревьев
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Слияние производится на основе самого большого дерева, то есть дерева,
составленного на наибольшем размере n-грамм.

Используйте защищённый метод
:py:meth:`lab_4_auto_completion.main.DynamicNgramLMTrie._copy_tree`,
чтобы получить главное дерево на основе его корневого узла.
На данном шаге обязательно использовать реализованный Вами ранее геттер
корневого узла модели
:py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.get_root`.

Для каждого дерева меньшего размера n-граммы в порядке их уменьшения
совершите слияние ветки с помощью защищённого метода
:py:meth:`lab_4_auto_completion.main.DynamicNgramLMTrie._merge_tree_level`,
реализованного ранее.

Шаг 9.3. Построить новое дерево на основе всех длин n-грамм
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Шаг 10. Создать ``BackOffGenerator`` для динамического дерева
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Шаг 11. Продемонстрировать результаты в ``start.py``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. important:: Выполнение Шагов 2-11 соответствует 10 баллам.
...
2. **Продемонстрируйте генерацию текста**. Для этого создайте экземпляр
...
3. **Добавьте второй корпус ``assets/ussr_letters.txt``** через метод
   :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.update`,
   чтобы расширить контекстную базу.

4. **Продемонстрируйте генерацию текста** на обновлённом дереве с теми же параметрами.

5. **Сравните качество генерации**:

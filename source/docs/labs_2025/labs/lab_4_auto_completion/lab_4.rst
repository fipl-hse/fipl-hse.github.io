...
.. figure:: assets/basic_tree.png
   ...
   Структура префиксного дерева для н-грамм `I love studying`, `love studying Python`,
   `I love eating`, `love eating apples`, `eating apples alone`, `apples with friends`.
...
В данной лабораторной работе вам необходимо создать 3 исключения:
...
Описание внутреннего атрибута: `self._root` - корневой узел, объект класса `TrieNode`. Значение
корневого узла по умолчанию равняется None.
...
Шаг 6. Реализовать префиксное дерево с поддержкой частотности
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Для повышения качества генерации текста необходимо учитывать не только
возможные продолжения последовательностей, но и их **частоту встречаемости** в корпусе.
Это требует доработки структуры узла и алгоритмов построения дерева.

Шаг 6.1. Определить узел с частотой
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Реализуйте класс :py:class:`lab_4_auto_completion.main.NGramTrieNode`, наследующийся от
:py:class:`lab_4_auto_completion.main.TrieNode`. Узел должен дополнительно хранить
параметр `_frequency` — относительную частоту соответствующего n-грамма в корпусе.
Частота задается при инициализации и может обновляться методом
:py:meth:`lab_4_auto_completion.main.NGramTrieNode.set_frequency`.

Также этот класс включает в себя следующие методы:

- :py:meth:`lab_4_auto_completion.main.NGramTrieNode.get_frequency`, возвращающий тот
  самый параметр `_frequency`
- :py:meth:`lab_4_auto_completion.main.NGramTrieNode.add_child`, который
  добавляет дочерний узел к текущему узлу дерева,
  получая на вход целочисленное значение для нового узла ребёнка. Дочерний узел должен
  представлять собой новый экземпляр класса :py:class:`lab_4_auto_completion.main.NGramTrieNode`
- :py:meth:`lab_4_auto_completion.main.NGramTrieNode.__str__`, возвращающий
  строковую репрезентацию узла n-грамм.

.. note:: Для имплементации метода :py:meth:`lab_4_auto_completion.main.NGramTrieNode.__str__`
          необходимо воспользоваться атрибутами родительского класса: `self._data`
          и `self._children`.

Шаг 6.2. Реализовать префиксное дерево с частотной информацией
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Реализуйте класс :py:class:`lab_4_auto_completion.main.NGramTrieLanguageModel`, наследующийся от
:py:class:`lab_4_auto_completion.main.PrefixTrie`. Его ключевой метод
:py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.build` строит модель префиксного
дерева из корпуса. Дерево должно:
...
- После построения автоматически вычислять **относительные частоты** всех n-грамм
  и сохранять их в соответствующих листовых узлах.

Другими словами, в начале из корпуса извлекаются все подпоследовательности
длины `n_gram_size` (n-граммы) с помощью скользящего окна и сохраняются в список `n_grams`.
После в :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.build` задействуются следующие методы,
которые необходимо реализовать:

- :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._collect_all_ngrams` — обход дерева
  для сбора всех сохранённых n-грамм, а после их передача в метод
  :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._fill_frequencies`

- :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._fill_frequencies` —
  вычисление и присвоение
  относительных частот узлам на основе статистики корпуса.

Также понадобится реализовать метод
:py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.get_n_gram_size`,
который возвращает значение размера n-граммы, заданное при создании экземпляра.

.. note:: Для имплементации метода
          :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._collect_all_ngrams` необходимо
          пройти по дереву, начиная с корня `self._root`, и собрать все последовательности токенов
          длиной `n_gram_size`. Этот метод возвращает список всех полных n-грамм,
          хранящихся в дереве. Например, возвращаемое значение может выглядеть так:

.. code:: py

   [(10, 25, 33), (25, 33, 41), (33, 41, 5)]

Каждая n-грамма представлена как кортеж целых чисел (tuple[int, ...]), где числа — это ID токенов.

Шаг 6.3. Поддержать обновление модели на новых данных
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Метод должен использовать те же вспомогательные функции,
что и :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.build`:

- :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._collect_all_ngrams` —
  для получения полного списка n-грамм после добавления новых данных,
- :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._fill_frequencies` —
  для обновления частот с учётом нового общего количества n-грамм.

.. Шаг 6.4. Реализовать контекстную генерацию следующего токена
.. ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. - Принимает произвольную последовательность токенов
.. - Берёт последние `n_gram_size - 1` токенов как **контекст**
.. - Находит все возможные следующие токены
.. - Возвращает **словарь**: {токен : его условная вероятность (частота) в данном контексте}

.. Если длина входа меньше `n_gram_size - 1`, то метод возвращает 'None'.
.. Если же контекст отсутствует в модели, то метод перехватывает исключение
.. ``TriePrefixNotFoundError`` и возвращает пустой словарь.

.. Для получения продолжений контекста внутри
.. :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.generate_next_token`
.. используется вспомогательный метод
.. :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.get_next_tokens`,
.. который возвращает словарь возможных следующих токенов и их относительных частот,
.. хранящихся в дочерних узлах контекста. Для сбора частот дочерних токенов
.. внутри :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.get_next_tokens` используется
.. метод :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._collect_sequences`.
.. Он обходит непосредственных потомков заданного узла и заполняет переданный словарь парами
.. {токен: частота}, взятыми из соответствующих дочерних узлов. Частоты устанавливаются методом
.. :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._fill_frequencies`
.. при построении или обновлении модели.

.. .. note:: В методе
..           :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel._collect_sequences`
..           необходимо использовать ``get_children()``, ``get_data()`` и
..           ``get_frequency()``, притом
..           дочерний узел должен существовать и являться экземпляром
..           :py:class:`lab_4_auto_completion.main.NGramTrieNode`, тогда
..           используется его частота, в противном случае
..           частота считается равной ``0.0``.

.. Для доступа к внутреннему узлу по префиксу предусмотрен метод
.. :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.get_node_by_prefix`.
.. Он ведёт себя аналогично :py:meth:`lab_4_auto_completion.main.PrefixTrie.get_prefix`
.. и выбрасывает исключение ``TriePrefixNotFoundError``, если префикс отсутствует.

.. Шаг 6.5. Завершить заполнение класса NGramTrieLanguageModel методом строковой репрезентации
.. ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. Реализовать метод
.. :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.__str__`, возвращающий
.. строковую репрезентацию
.. класса :py:class:`lab_4_auto_completion.main.NGramTrieLanguageModel`. Возвращаемая
.. строка должна иметь
.. ВНИМАНИЕ ЗДЕСЬ ТОЧНО ОШИБКА; И ВООБЩЕ С РЕПР НАДО РАЗБИРАТЬСЯ
.. следующий вид: ``NGramTrieLanguageModel(n_gram_size, leaf_nodes)``.

Шаг 6.6. Продемонстрировать результаты в ``start.py``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. important:: Выполнение Шагов 2-6 соответствует 8 баллам.

Продемонстрируйте результат в функции ``main()`` модуля ``start.py``:

- Постройте модель на основе корпуса ``assets/Harry_Potter.txt``
- Добавьте второй корпус ``assets/ussr_letters.txt`` через метод
  :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.update`,
  чтобы расширить контекстную базу
- Продемонстрируйте генерацию текста с использованием
  :py:meth:`lab_4_auto_completion.main.NGramTrieLanguageModel.generate_next_token` и жадного выбора
  наиболее вероятного продолжения (в цикле до достижения заданной длины)
- Сравните качество генерации до и после объединения двух корпусов.

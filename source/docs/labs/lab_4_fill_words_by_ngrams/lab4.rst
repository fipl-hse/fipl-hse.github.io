...
генерации уникальных идей и контента. При этом очевидно, что степень случайности
...
Ее идея заключается
в том, что при выборе следующего токена мы ограничиваемся набором наиболее
вероятных токенов, порог вхождения для которых определяется параметром.
...
:py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.WordProcessor`.
Данный класс необходимо наследовать от
:py:class:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor`, поскольку их логика
...
Шаг 2.1. Токенизация текста
"""""""""""""""""""""""""""

Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.WordProcessor._tokenize`,
...
унаследованном от класса :py:class:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor`.
...
Здесь ``'<eos>'`` - специальный токен конца предложения.
...
Шаг 2.2. Добавление токена в хранилище
""""""""""""""""""""""""""""""""""""""
...
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.WordProcessor._put`.
...
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.WordProcessor._postprocess_decoded_text`,
...
Здесь ``'<eow>'`` - токен конца предложения.
...
лабораторной работе:
:py:class:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel`.
...
:math:`\sum_{i=1}^{N}p(w_{i}|context) \geq P`

:math:`\sum_{i=1}^{N - 1}p(w_{i}|context) < P`
...
класс :py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.TopPGenerator`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.TopPGenerator.run`,
...
3. Выявить `N` минимальное количество наиболее вероятных токенов, чья суммарная последовательность
   превышает заданный `P` или равна ему.
...
жадный алгоритм, алгоритм `Beam Search` и алгоритм `Top P`.

Необходимо объявить сущность, хранящую данные виды генерации в качестве атрибутов:
:py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.GeneratorTypes`.
...
Шаг 4.3. Получение названия алгоритма генерации
"""""""""""""""""""""""""""""""""""""""""""""""""
...
Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.GeneratorTypes.get_conversion_generator_type`
...
Шаг 4.2. Хранение оценки генерации текста
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
последовательности: :py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.GenerationResultDTO`.
...
Шаг 4.4. Получение значения метрики качества
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.GenerationResultDTO.get_perplexity`,
который возвращает значение перплексии.

Шаг 4.4. Получение оцениваемой последовательности
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Тем не менее, потребность узнать данное значение существует, для этого реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.GenerationResultDTO.get_text`,
который возвращает текст.

Шаг 4.4. Получение идентификатора типа алгоритма генерации
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Тем не менее, потребность узнать данное значение существует, для этого реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.GenerationResultDTO.get_type`,
который возвращает идентификатор алгоритма генерации.

Шаг 4.5. Получение отчета об оценке качества генерации
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Наконец, необходимо реализовать формирование отчета о полученном результате.

Для этого реализуйте магический метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.GenerationResultDTO.__str__`,
...
Допустим, экземпляр был инициализирован следующим образом:
...
Шаг 4.6. Объявление сущности для оценки сгенерированного текста
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Наконец, перейдем к реализации класса, ответственного за проведение
сравнительного анализа:
:py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.QualityChecker`.
...
Шаг 4.7. Подсчет метрики качества последовательности
""""""""""""""""""""""""""""""""""""""""""""""""""""

Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.QualityChecker._calculate_perplexity`.
...
Продублируем формулу перплексии:
...
.. note:: Если аргументы имеют некорректный тип данных, то есть не
          ...
          либо если сгенерированный текст не начинается с изначального контекста,
          а также если вызываемые методы
          возвращают ``None``, необходимо поднять исключение ``ValueError``.

Шаг 4.8. Реализовать логику сравнительного анализа
""""""""""""""""""""""""""""""""""""""""""""""""""

Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.QualityChecker.run`,
...
:py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.GenerationResultDTO`.
...
Шаг 4.9. Продемонстрировать работу реализации в ``start.py``
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Для `Top P` алгоритма качестве значения `P` используйте `0.5`, в качестве параметра
ширины луча для алгоритма `Beam Search` используйте `5`.
Начните генерацию со строки ``'The'``, при сравнительном
анализе используйте длину последовательности `100`.
...
Шаг 5.1. Объявление сущности экзаменатора
"""""""""""""""""""""""""""""""""""""""""

Далее необходимо определить класс
:py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.Examiner`,
...
Шаг 5.2. Загрузка вопросов и ответов
""""""""""""""""""""""""""""""""""""

Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.Examiner._load_from_json`,
...
Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.Examiner.provide_questions`,
возвращающий вопросы экзамена. Метод не должен возвращать ответы!

Шаг 5.4. Выставление оценки
"""""""""""""""""""""""""""

Реализуйте метод
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.Examiner.assess_exam`,
...
Шаг 5.5. Объявление сущности студента:
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Далее необходимо определить класс
:py:class:`stubs.2023.lab_4_fill_words_by_ngrams.main.GeneratorRuleStudent`.
...
Шаг 5.6. Составление ответов на задания
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Логику обработки заданий необходимо реализовать в методе
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.GeneratorRuleStudent.take_exam`.
...
Шаг 5.7. Запрос типа алгоритма заполнения
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
:py:meth:`stubs.2023.lab_4_fill_words_by_ngrams.main.GeneratorRuleStudent.get_generator_type`.
...
- `Заполнение пропусков
  <https://github.com/stlong0521/missing-word>`__

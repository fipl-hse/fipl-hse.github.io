...
:py:class:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor`, который Вы реализуете в
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._tokenize`,
...
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._put`.
...
Для этого реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.get_id`.
...
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.get_token`,
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.encode`,
который кодирует текст. Он обязательно должен вызывать методы
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._tokenize`,
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._put` и
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.get_id`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._decode`,
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._postprocess_decoded_text`,
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.decode`,
...
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._decode` и
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor._postprocess_decoded_text`.
...
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.get_end_of_word_token`,
...
Класс :py:class:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel`
...
Создайте класс :py:class:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel._extract_n_grams`,
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.build`,
...
n-граммы в контексте. Метод обязательно должен вызывать
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel._extract_n_grams`.

В данной работе для подсчета вероятности вам необходимо использовать формулу
условной вероятности, которая имеет следующий вид:

.. math::  \frac{P(w_{1},w_{2})}{P(w_{2})}

**Условная вероятность** :math:`{P(w_{1}|w_{2}}` – вероятность наступления одного события (в
данном случае события :math:`P(w_{1})`) при условии, что другое событие (событие
:math:`P(w_{2})`) уже произошло.

То есть, если мы говорим в контексте n-грамм, то условная вероятность - вероятность
появления токена с учётом некоторого контекста.

Например, предположим, что у нас есть контекст `(4, 3, 2, 1, 7)` и мы хотим узнать вероятность
того, что следующий токен - `(5)`, тогда необходимо найти :math:`{P((5)|(4, 3, 2, 1, 7)}`.

Один из способов оценить данную вероятность — это посчитать относительную частоту, то есть
сколько раз мы видим контекст `(4, 3, 2, 1, 7)` и сколько раз за этим контекстом следует `(5)`,
следующим образом:

.. math::  {P((5)|(4, 3, 2, 1, 7)} = \frac{C((4, 3, 2, 1, 7, 5))}{C((4, 3, 2, 1, 7))}

В случае подсчета вероятности появления последнего токена данной n-граммы в контексте,
задаваемом n-граммой:

* :math:`P(w_{1},w_{2})` - количество вхождений n-граммы, то есть её абсолютная частота;
* :math:`P(w_{2})` - сколько раз в корпусе встречаются n-граммы, имеющие такое
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.get_n_gram_size`,
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.generate_next_token`.
...
создайте класс :py:class:`stubs.2023.lab_3_generate_by_ngrams.main.GreedyTextGenerator`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.GreedyTextGenerator.run`,
...
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.encode` и
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.decode`,
а также методы
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.get_n_gram_size` и
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.generate_next_token`.
...
Для этого создайте класс :py:class:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearcher`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearcher.get_next_token`,
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearcher.continue_sequence`,
...
становятся настолько маленькими, что все обращаются в ``0``.
В таком случае возникает проблема максимального правдоподобия.
Чтобы избежать побочного эффекта от данной ситуации, необходимо применить логарифмирование.
Логарифм преобразует результат из произведения в сумму,
что значительно упрощает дальнейший анализ.

Результаты не являются численно стабильными: они имеют тенденцию либо быстро сходиться к
нулю, либо к бесконечности, в зависимости от того,
принимает ли вероятность значение меньше или больше ``1``.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearcher.prune_sequence_candidates`,
...
:py:class:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearchTextGenerator`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearchTextGenerator._get_next_token`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearchTextGenerator.run`,
...
1. Создайте словарь с последовательностями-кандидатами, ключом которого изначально
   является закодированное начало последовательности, а значением - частота
   закодированной последовательности, принимающая значение ``0.0``.
2. Получите топ-n (где n - ширина луча) букв-кандидатов
   для продолжения текущей последовательности.
...
а также методы
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearcher.get_next_token`,
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearcher.continue_sequence` и
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BeamSearcher.prune_sequence_candidates`.
...
четыре, то после их сортировки необходимо было оставить количество, заданное шириной луча).
...
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.fill_from_ngrams`.
...
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.set_n_grams`.
...
Создайте класс :py:class:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModelReader`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModelReader.load`,
который позволяет получить языковую модель для определенного размера n-грамм.
...
Если заданный размер n-грамм соотносится с длиной n-грамм из файла,
то необходимо кодировать n-грамму из языкового профиля.
...
Метод обязательно должен вызывать методы
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.TextProcessor.get_id` и
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.set_n_grams`.
...
Для этого реализуем метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModelReader.get_text_processor`,
...
Создайте класс :py:class:`stubs.2023.lab_3_generate_by_ngrams.main.BackOffGenerator`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BackOffGenerator._get_next_token`,
который возвращает словарь, ключами которого являются буквы-кандидаты,
а значениями - частоты n-грамм, из которых данная буква была выявлена по контексту.
В данном методе необходимо использовать методы
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.get_n_gram_size` и
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.NGramLanguageModel.generate_next_token`.
...
Реализуйте метод
:py:meth:`stubs.2023.lab_3_generate_by_ngrams.main.BackOffGenerator.run`,
...
прекращается. Метод возвращает сгенерированный текст в виде строки.

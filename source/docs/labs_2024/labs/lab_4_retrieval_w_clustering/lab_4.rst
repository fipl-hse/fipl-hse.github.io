...
тестов запустится при проверке Вашего Pull Request.
...
Например, пусть функция принимает на вход следующий текст:

``'Привет! Как твои дела?\nХорошо. Как твои дела?\nУ меня всё отлично\nКак
ты смотришь на то, чтобы сходить попить кофе?'``.

Тогда функция вернет нам следующий список строк, являющихся абзацами:

``['Привет! Как твои дела?', 'Хорошо. Как твои дела?',
'У меня всё отлично', 'Как ты смотришь на то, чтобы сходить попить кофе?']``

.. note:: Если на вход в функцию подаётся пустой аргумент,
...
При инициализации данного класса заполните атрибуты дефолтными значениями.
...
создание таких баз данных позволяет хранить документы и их векторы в одном месте.
...
.. important:: Атрибуты ``self._tokenizer`` и ``self._vectorizer`` являются защищёнными, а
               ...
               почему так сделано, и поделитесь ответом с ментором во время защиты лабораторной работы.
...
.. note:: Если на вход подаётся пустой аргумент, некорректное значение количества
          релевантных документов или вызываемые методы возвращают значение ``None``,
          то необходимо поднять исключение ``ValueError``.
...
Шаг 6. Продемонстрировать результат в ``start.py``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. important:: Выполнение Шагов 2-6 соответствует 6 баллам.

Продемонстрируйте результат работы токенизации по абзацам,
векторизации с помощью BM25, заполнения базы данных и ранжирования документов по
релевантности в функции ``main()`` модуля ``start.py``.
...
**Алгоритм кластеризации** позволяет группировать данные в наборы объектов,
...
Помимо скорости, кластеризация, при достаточно большой базе данных, может улучшить качество
...
как наиболее подходящие среди всех документов.
...
Работу алгоритма кластеризации можно разделить на два основных этапа:
...
2. **Инференс (Inference)** – использование созданной модели для предсказания принадлежности
   новых данных к определённым кластерам.

Этап тренировки описывается следующими шагами:

1. **Инициализация центроидов**. На данном шаге случайным образом выбираются ``k``
   векторов из базы данных. Данные векторы служат начальными центроидами для кластеров.
2. **Цикл обучения**.
...
После того как модель была обучена, её можно использовать для предсказания того, к какому кластеру
...
.. important:: Атрибуты ``self.__centroid`` и ``self.__indices`` являются приватными.
               ...
               во время защиты лабораторной работы.
...
``len`` и передав экземпляр класса в качестве аргумента.
...
.. note:: Если на вход подаётся пустой аргумент или аргумент ``index`` принимает
          отрицательное значение, то необходимо поднять исключение ``ValueError``.
...
.. important:: Атрибуты ``self._db`` и ``self._n_clusters`` являются защищёнными, а
...
Шаг 7.2.3. Обучить алгоритм кластеризации
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
2. Получить индексы тех документов, которые содержатся в ближайшем к запросу
   пользователя кластере.
3. Получить векторы документов из кластера по индексу.
...
документов равное ``2``. Тогда метод вернёт следующий результат:
...
.. important:: Атрибут ``self._db`` является защищённым, а
...
Шаг 9. Продемонстрировать результаты в ``start.py``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Шаг 14. Продемонстрировать результаты в ``start.py``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

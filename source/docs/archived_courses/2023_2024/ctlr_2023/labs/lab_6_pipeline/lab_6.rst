...
.. note:: The `lab_6_pipeline.pipeline.CorpusManager`
...
The `lab_6_pipeline.pipeline.UDPipeAnalyzer._bootstrap` method
...
Wrapper does not accept any arguments during initialization, but calls protected method
`lab_6_pipeline.pipeline.StanzaAnalyzer._bootstrap`,
...
must possess a method for producing a file with ``.conllu`` extension with
retrieved linguistic markup.
Method `lab_6_pipeline.pipeline.StanzaAnalyzer.to_conllu`
...
The method accepts
one instance of `core_utils.ctlr.article.article.Article` as an argument.
...
`lab_6_pipeline.pipeline.TextProcessingPipeline`
with `lab_6_pipeline.pipeline.StanzaAnalyzer`,
...
`lab_6_pipeline.pipeline.StanzaAnalyzer.from_conllu` method.

Its responsibility is the opposite of that of
`lab_6_pipeline.pipeline.StanzaAnalyzer.to_conllu` method: it
...
3. Calculates frequencies of each part of speech via protected method
   `lab_6_pipeline.pipeline.POSFrequencyPipeline._count_frequencies`,

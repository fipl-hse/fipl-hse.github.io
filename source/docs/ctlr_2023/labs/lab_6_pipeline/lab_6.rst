...
.. note:: The :py:class:`stubs.labs.lab_6_pipeline.pipeline.CorpusManager` knows where are the files,
...
.. note:: It is mandatory to get articles with the
          ...
          :py:func:`stubs.core_utils.ctlr.article.io.from_raw` function.

.. tip:: Try to implement
         :py:meth:`stubs.labs.lab_6_pipeline.pipeline.TextProcessingPipeline.run`
         in a way that it goes through the articles collected by
         :py:meth:`stubs.labs.lab_6_pipeline.pipeline.CorpusManager.get_articles`
         and then writes to the file as a processed article using the
         :py:func:`stubs.core_utils.ctlr.article.io.to_cleaned` function. At least you
...
:py:meth:`stubs.labs.lab_6_pipeline.pipeline.UDPipeAnalyzer._bootstrap`, which is responsible for
...
:py:meth:`stubs.labs.lab_6_pipeline.pipeline.UDPipeAnalyzer._bootstrap` method must read the UDPipe
...
:py:meth:`stubs.labs.lab_6_pipeline.pipeline.UDPipeAnalyzer._bootstrap` returns the resulting model,
...
:py:class:`stubs.core_utils.ctlr.article.article.Article` instance. The method accepts
one instance of :py:class:`stubs.core_utils.ctlr.article.article.Article` as an argument. It is
presumed that the given article object has a filled attribute with CoNLL-U markup.
The method thus uses interface of the :py:class:`stubs.core_utils.ctlr.article.article.Article`
...
Wrapper does not accept any arguments during initialization, but calls protected method
:py:meth:`stubs.labs.lab_6_pipeline.pipeline.StanzaAnalyzer._bootstrap`, which is responsible for
downloading and initializing the Stanza model.
The
:py:meth:`stubs.labs.lab_6_pipeline.pipeline.StanzaAnalyzer._bootstrap` method must
...
must possess a method for producing a file with ``.conllu`` extension with
retrieved linguistic markup.
Method :py:meth:`stubs.labs.lab_6_pipeline.pipeline.StanzaAnalyzer.to_conllu` does not perform
any analysis but operates fields of
...
:py:class:`stubs.labs.lab_6_pipeline.pipeline.TextProcessingPipeline`
with :py:class:`stubs.labs.lab_6_pipeline.pipeline.StanzaAnalyzer`, then you must have made a mistake
...
3. Calculates frequencies of each part of speech via protected method
   :py:meth:`stubs.labs.lab_6_pipeline.pipeline.POSFrequencyPipeline._count_frequencies`, which
...
5. Visualizes frequencies in a form of images with names following
   ...
   :py:func:`stubs.core_utils.ctlr.visualizer.visualize`.
...
which was obtained using :py:meth:`stubs.core_utils.ctlr.pipeline.LibraryWrapper.from_conllu` method.

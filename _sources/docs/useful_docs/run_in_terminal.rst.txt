...
Попробуем запустить файл ``start.py`` лабораторной работы ``lab_N_lab_name``
в проекте ``2-level-labs``.

1. Откройте терминал (Windows: ``PowerShell``, macOS: ``Terminal``).

2. Перейдите в папку проекта ``cd C:\<path-to-folder>\2-level-labs`` (полный путь зависит от того, где находится проект).
...
5. Запустите программу через команду из корня проекта ``python lab_N_lab_name/start.py``.
...
Для этого выполним шаги 1-4 из :ref:`блока о запуске простой программы <running-code>`.

Затем установим зависимости из ``requirements.txt`` с помощью команды 
``python -m pip install -r requirements.txt``.

Наконец, запустим программу с помощью ``python lab_N_lab_name/start.py``.
...
Python не распознан как имя командлета
--------------------------------------
...
Если Python установлен, удостоверьтесь, что он есть в путях компьютера
``Path`` и ``PYTHONPATH``. Воспользуйтесь инструкцией :ref:`pythonpath_on_windows`.
